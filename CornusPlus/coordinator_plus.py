...
import time
import atexit
from utils_plus import send_message, receive_message, logOnce, write_local, read_local, termination_protocol
...
class Coordinator:
    ...
    def start_txn(self, txn):
        ...
        print(f"Transaction {txn}: {ret}. Time Taken: {(time_taken*1000):.4f}")
                        ...
                        if not message or message['txn_id'] != txn:
                            ...
                            for s in sockets_list:
                                if s != self.coordinator_socket:  # coordinator socket still needed for next txn
                                    s.close()
                            print("Coordinator express COMMIT")
                            return COMMIT
                        else:
                            decision = ABORT
                            break
